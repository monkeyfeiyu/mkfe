...
const KEY_LIST = [
    "OfDCluqiAXc3NfyEiBphmNk3MNMwvQVz",
        ...
        tinify.validate((e) => {
            if (e) {
                ...
                const compressionCount =
                    CompressionsOneMonth - tinify.compressionCount;
                bar.tick(1, {
...
(async () => {
    try {
        ...
        const TotalCompressions = CompressionsList.reduce(
            (prev, cur) => prev + cur,
            0
        );
        ...
        const KEY_LIST_Compressions = CompressionsList.reduce(
            (prev, cur, index) => {
        ...
        const selectedImageList = imageList.filter((v) =>
            v.startsWith(answers.entry)
        );

        if (selectedImageList.length > TotalCompressions) {
            throw new Error(
                `共${selectedImageList.length}张图片, 账号剩余可用: ${TotalCompressions}`
            );
        }
